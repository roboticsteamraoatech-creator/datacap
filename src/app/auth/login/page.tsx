  ...
  const { mutate: submitMutate, isPending } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (data && data.requiresEmailVerification) {
        ...
        // Get the role - it's uppercase from API
        const userRole = data.user.role
        
        console.log('🚀 User role detected:', userRole)
        console.log('🚀 Full user data:', data.user)
        
        // Check for SUPER_ADMIN (uppercase as per API response)
        if (userRole === 'SUPER_ADMIN') {
          toast({ 
            title: "LOGIN SUCCESSFUL!",
            description: "Welcome to the super admin dashboard!"
        ...
        // Check for ORGANIZATION (uppercase as per API response)
        else if (userRole === 'ORGANIZATION' || userRole === 'ORGANISATION') {
          toast({ 
            title: "LOGIN SUCCESSFUL!",
            description: "Welcome to the admin dashboard!"
          })
